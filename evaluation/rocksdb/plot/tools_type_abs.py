...
def parse_io_perf_file(bench_dirname,pattern):
    ...
    # for filename in os.listdir(f"{bench_dirname}"):
            if filename.endswith(".tsv"):
                ...
                # with open(f"{bench_dirname}/{filename}", "r") as file:
                    ...
                    lat_p99 += float(parts[13])
                    lat_p999 += float(parts[14])
                    lat_p9999 += float(parts[15])
                    lat_pmax += float(parts[16])
                    count += 1
    ...
    lat_p999 /= count
    lat_p9999 /= count
    return iops,thru_put, lat_avg, lat_p50, lat_p99, lat_p999, lat_p9999,lat_pmax,key_str[0],key_str[1]
...
def parse_io_perfs(dirname, name,pattern):
    ...
    iops, thru_put, lat_avg, lat_p50,  lat_p99, lat_p999, lat_p9999,lat_pmax,vm_num,cycle_cnt = parse_io_perf_file(
        f"{dirname}",pattern)
    ...
    if(type=="host-sic"):
            ...
            type="hypath"
    elif(type=="spdk-vhost"):
            type="SPDK-Vhost"
    elif(type=="spdk-vhost-1"):
            type="Vhost-C1"
    elif(type=="spdk-vhost-4"):
            type="Vhost-C4"
    elif(type=="spdk-vhost-8"):
            type="Vhost-C8"
    elif(type=="cx5-blk"):
            type="cx5"
