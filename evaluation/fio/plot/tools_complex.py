...
ORDERED_NAMES = ["Non-offloading", "Naive-Offloading","Virtio","HyPath"]
    ...
    for name in ORDERED_NAMES:
        if name in names:
            for item in io_perfs_iops[name]:
                for base_item in io_perfs_iops["HyPath"]:
        ...
        if(y_key == "iops"):
            ...
            for a, b in zip([i + offset for i in ticks], y):
                plt.text(a+0.005, b+120, '%.1f' % float(b), ha='center',
                         va='bottom', fontsize=18, rotation=90, clip_on=False)
            plt.set_ylim(bottom=0, top=5500)
            plt.set_xticks(ticks)
            plt.set_xticklabels(x, fontsize=20,color="black")
            plt.yaxis.set_major_locator(MultipleLocator(1000))
        elif(y_key == "latency"):
            for a, b in zip([i + offset for i in ticks], y):
            ...
            for a, b in zip([i + offset for i in ticks], y):
                plt.text(a+0.01, b+0.3, '%.1f' % float(b), ha='center',
                         va='bottom', fontsize=20, rotation=90, clip_on=False)
            # plt.set_ylim(bottom=0, top=180)
            # plt.tick_params(axis='x', pad=50)
...
def plot_result_single(title, names, cpu_loads_iops, io_perfs_iops, cpu_loads_bw, io_perfs_bw, x_key, x_label, resfile):
    ...
    for name in ORDERED_NAMES:
        if name in names:
            ...
            if(title == "IOPS"):
                ...
                plot_bar(normalized_iops,  io_perfs_iops[name], x_key, "normalized_latency", name, offset * width, width, COLORS[name], HATCHS[name], True,data)
                offset += 1.0
            elif(title == "cutil-iops" or title=="cutil-bw"):
    ...
    normalized_iops.legend(ncol=5,fontsize=19,frameon=True,handletextpad=0.1,handlelength=1,borderpad=0.3,bbox_to_anchor=(0.85,1.25),edgecolor="black",shadow=False,fancybox=False)

    res = resfile+"/"+title+"_complex"
    fig.savefig(f"{res}.png", dpi=300)
    fig.savefig(f"{res}.pdf", format="pdf")
